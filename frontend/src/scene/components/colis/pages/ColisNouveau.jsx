...
import { Button, message, Modal, Input, Card, Row, Col, Statistic, Tag, Typography, Tooltip } from 'antd';
import { useDispatch, useSelector } from 'react-redux';
import { getNouveauColis } from '../../../../redux/apiCalls/colisApiCalls';
import { BsUpcScan } from 'react-icons/bs';
import { FaBoxes, FaDownload, FaPrint as FaPrintIcon } from 'react-icons/fa';
import { IoMdRefresh } from 'react-icons/io';
import { CopyOutlined, CheckOutlined, EnvironmentOutlined, CalendarOutlined, PhoneOutlined, ShopOutlined, TagOutlined, EditOutlined } from '@ant-design/icons';
...
function ColisNouveau() {
  const { theme } = useContext(ThemeContext);
  const dispatch = useDispatch();
  const [searchQuery, setSearchQuery] = useState('');
  const [selectedRowKeys, setSelectedRowKeys] = useState([]);

  const { nouveauColisList } = useSelector(state => state.colis);
  const { user } = useSelector(state => state.auth);

  useEffect(() => {
    dispatch(getNouveauColis());
    window.scrollTo(0, 0);
  }, [dispatch]);
  ...
  // Filtered data
  const filteredColis = nouveauColisList.colis.filter(colis => {
          ...
          <div className="content" style={{ backgroundColor: theme === 'dark' ? '#001529' : '#fff' }}>
            <div className="page-content-header">
              ...
              <div style={{
                ...
                <Button icon={<IoMdRefresh />} type="primary" onClick={() => dispatch(getNouveauColis())} loading={nouveauColisList.loading}>Refresh</Button>
                ...
                {user?.role === 'admin' && (
                  <Button
                    type="primary"
                    style={{ minWidth: 220, background: '#0ea5e9', borderColor: '#0ea5e9' }}
                    disabled={selectedRowKeys.length === 0 || nouveauColisList.loading}
                    loading={nouveauColisList.loading}
                    onClick={async () => {
                      ...
                      try {
                        ...
                        await request.put('/api/colis/statu/update', {
                          ...
                          new_status: 'attente de ramassage'
                        }, config);
                        message.success('Statut changé en "attente de ramassage" pour les colis sélectionnés.');
                        setSelectedRowKeys([]);
                        dispatch(getNouveauColis());
                      } catch (err) {
                  ...
                  >
                    Attente de Ramassage ({selectedRowKeys.length})
                  </Button>
                )}
              </div>
            </div>
            <Row gutter={[16, 16]} style={{ marginBottom: '24px' }}>
              <Col xs={24} sm={12} md={6}>
                <Card>
                  <Statistic title="Total Colis" value={nouveauColisList.total} valueStyle={{ color: theme === 'dark' ? '#60a5fa' : '#3b82f6' }} prefix={<FaBoxes />} />
                </Card>
              </Col>
            </Row>
            ...
            {nouveauColisList.loading ? (
              <div style={{ textAlign: 'center', padding: '40px' }}>
                <IoMdRefresh style={{ fontSize: '24px', color: theme === 'dark' ? '#60a5fa' : '#3b82f6' }} className="spin-refresh" />
                <div style={{ marginTop: '16px' }}>Chargement des colis...</div>
              </div>
            ) : nouveauColisList.error ? (
              <div style={{ textAlign: 'center', padding: '40px', color: '#ff4d4f' }}>
                <FaBoxes style={{ fontSize: '24px', color: '#ff4d4f' }} />
                <div style={{ marginTop: '16px' }}>{nouveauColisList.error}</div>
              </div>
            ) : (
              <TableDashboard
                column={columns}
